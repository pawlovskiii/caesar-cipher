...
function caesar13(input) {
	...
	for (let i = 0; i < splittedInputToArray.length; i++) {
		const indexLowerCaseAlphabetFromInput = lowerCaseAlphabet.indexOf(splittedInputToArray[i]);
		const indexUpperCaseAlphabetFromInput = upperCaseAlphabet.indexOf(splittedInputToArray[i]);
			...
			if (indexLowerCaseAlphabetFromInput + 13 < 26) {
				shiftedOutputArray.push(
					lowerCaseAlphabet.charAt(
						indexLowerCaseAlphabetFromInput + 13,
					),
				);
			}
			shiftedOutputArray.push(
				lowerCaseAlphabet.charAt(
					indexLowerCaseAlphabetFromInput + 13 - 26,
				),
			);
		} else if (indexUpperCaseAlphabetFromInput !== -1) {
			if (indexUpperCaseAlphabetFromInput + 13 < 26) {
				shiftedOutputArray.push(
					upperCaseAlphabet.charAt(
						indexUpperCaseAlphabetFromInput + 13,
					),
				);
			}
			shiftedOutputArray.push(
				upperCaseAlphabet.charAt(
					indexUpperCaseAlphabetFromInput + 13 - 26,
				),
			);
		} else {
